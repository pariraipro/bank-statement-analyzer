...
const state={transactions:[],analysis:null,name:'',demo:true,issues:[],metadataRows:0,dismissed:new Set(),page:0};
...
function setData(transactions,name,demo,issues=[],metadataRows=0){const analysis=analyze(transactions);Object.assign(state,{transactions,analysis,name,demo,issues,metadataRows,dismissed:new Set(),page:0});render()}
...
function render(){
 ...
 const notice=$('notice');notice.hidden=!state.issues.length&&!state.metadataRows;
 if(!notice.hidden){notice.innerHTML=`${state.issues.length?`<strong>${state.issues.length} row${state.issues.length===1?'':'s'} could not be analyzed.</strong> Totals include only the ${state.transactions.length} valid transaction rows. <details><summary>Review excluded rows</summary><ul>${state.issues.map(i=>`<li>Line ${i.line}: ${escape(i.reason)}</li>`).join('')}</ul></details>`:''}${state.metadataRows?`<div>${state.metadataRows} introductory record${state.metadataRows===1?'':'s'} before the header were skipped.</div>`:''}`}
...
async function chooseFile(file){
 if(!file)return;const request=++fileRequest;parsedFile=null;$('mapping-section').hidden=true;$('upload-error').hidden=true;
 if(!/\.csv$/i.test(file.name)){errorUpload('Please choose a .csv file. Excel workbooks and PDFs are not supported.');return}
